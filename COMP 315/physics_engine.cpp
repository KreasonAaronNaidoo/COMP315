...
void physics_engine::col_dec_bullet_to_asteroid(){
    ...
    for(int a = 0; a < v_asteroid.size(); a++){
        for(int b = 0; b < player1->v_bullet.size(); b++){
            float d = sqrt(((player1->v_bullet[b]->sx - v_asteroid[a]->sx) * (player1->v_bullet[b]->sx - v_asteroid[a]->sx))
            + ((player1->v_bullet[b]->sy - v_asteroid[a]->sy) * (player1->v_bullet[b]->sy - v_asteroid[a]->sy)) +
            ((player1->v_bullet[b]->sz - v_asteroid[a]->sz) * (player1->v_bullet[b]->sz - v_asteroid[a]->sz)));

            // check bullets hitting the world boundry
            if((player1->v_bullet[b]->sx * player1->v_bullet[b]->sx) + (player1->v_bullet[b]->sy * player1->v_bullet[b]->sy)
               + (player1->v_bullet[b]->sz * player1->v_bullet[b]->sz) >= 10000){
